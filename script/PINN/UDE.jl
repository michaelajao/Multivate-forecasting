...
function create_NN(input_size, hidden_size, output_size)
    nn = Chain(
        ...
        Dense(hidden_size, hidden_size, Lux.tanh_fast),
        Dense(hidden_size, hidden_size, Lux.tanh_fast),
        Dense(hidden_size, output_size, Lux.sigmoid_fast))
...
res = Optimization.solve(optprob, ADAM(0.0001), callback=callback, maxiters=15000)
...
res1 = Optimization.solve(optprob2, BFGS(; initial_stepnorm=0.01, linesearch=LineSearches.BackTracking()), callback=callback, maxiters=1000)
println("Final training loss after $(length(losses)) iterations: $(losses[end])")


# Visualize the training loss

fig = with_theme(makie_theme()) do
    fig = Figure(resolution = (800, 600))
    ax = Axis(fig[1, 1], xlabel = "Iterations", ylabel = "Loss", title = "Training Loss")
    lines!(ax, losses, color = :blue, linewidth = 2, label = "Loss")
    fig
end
