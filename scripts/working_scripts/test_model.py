...
data = load_and_preprocess_data("../../data/hos_data/merged_data.csv", areaname="South West", recovery_period=21, start_date="2020-04-01", end_date="2020-08-31")
...
def prepare_tensors(data, population, device):
    """Prepare tensors for training."""
    t = tensor(range(1, len(data) + 1), dtype=torch.float32).view(-1, 1).to(device).requires_grad_(True)
    S = tensor(data["susceptible"].values / population, dtype=torch.float32).view(-1, 1).to(device)
    I = tensor(data["active_cases"].values / population, dtype=torch.float32).view(-1, 1).to(device)
    R = tensor(data["recovered"].values / population, dtype=torch.float32).view(-1, 1).to(device)
    D = tensor(data["new_deceased"].values / population, dtype=torch.float32).view(-1, 1).to(device)
    H = tensor(data["covidOccupiedMVBeds"].values / population, dtype=torch.float32).view(-1, 1).to(device)
    return t, S, I, R, D, H

def split_and_scale_data(data, population, train_size, features, device):
    """Split and scale data into training and validation sets."""
    train_data = data.iloc[:train_size]
    val_data = data.iloc[train_size:]

    t_train, S_train, I_train, R_train, D_train, H_train = prepare_tensors(train_data, population, device)
    t_val, S_val, I_val, R_val, D_val, H_val = prepare_tensors(val_data, population, device)
...
tensor_data = split_and_scale_data(data, population, train_size, features, device)

# plot susceptible from the training data
plt.figure(figsize=(10, 5))
plt.plot(data["date"][:train_size], tensor_data["train"][3].cpu().numpy(), label="Susceptible (Training)", color="blue")
plt.xlabel("Date")
plt.ylabel("Susceptible")
plt.title("Susceptible Population (Training Data)")
plt.xticks(rotation=45)
plt.legend()
plt.show()
...
def pinn_loss(tensor_data, params, model_output, t, index=None):
    ...
    dsDt = -beta * S_pred * I_pred
    dIdt = beta * S_pred * I_pred - gamma * I_pred
    dRdt = gamma * I_pred
    dDdt = delta * I_pred
    dHdt = gamma * I_pred
...
model = SEIRDNet(num_layers=5, hidden_neurons=20).to(device)
param_net = ParameterNet(num_layers=5, hidden_neurons=20).to(device)
...
scheduler = StepLR(model_optimizer, step_size=6000, gamma=0.89)
...
plt.figure(figsize=(10, 5))
plt.plot(data["date"], model_output.detach().cpu().numpy()[:, 4] * population, label="ICU Bed Demand (Model)", color="red")
plt.xlabel("Date")
...
# Scale back the predictions to the original population scale
predicted_data = model_output.detach().cpu().numpy() * population
...
plt.figure(figsize=(10, 5))
plt.plot(data["date"], data["covidOccupiedMVBeds"], label="ICU Bed Demand (Data)", color="blue")
plt.plot(data["date"], H_pred, label="ICU Bed Demand (PINN Model)", color="red")
...
beta = params[0].detach().cpu().numpy()
