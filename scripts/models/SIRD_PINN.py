...
class SIRDNet(nn.Module):
    ...
    def __init__(self, inverse=False, init_beta=None, init_gamma=None, init_mu=None, retain_seed=42, num_layers=4, hidden_neurons=20):
        ...
        if inverse:
            ...
            self._mu = nn.Parameter(torch.tensor([init_mu if init_mu is not None else torch.rand(1)], device=device), requires_grad=True)
        else:
            self._beta = None
            self._gamma = None
            self._mu = None
...
def pinn_loss(tensor_data, model, t, N, device):
    ...
    S_pred, I_pred, R_pred, D_pred = model_output[:, 0], model_output[:, 1], model_output[:, 2], model_output[:, 3]
    ...
    data_loss_total = torch.mean((S_train - S_pred[:len(S_train)]) ** 2 + (I_train - I_pred[:len(I_train)]) ** 2 + (R_train - R_pred[:len(R_train)]) ** 2 + (D_train - D_pred[:len(D_train)]) ** 2)
    ...
    loss = data_loss_total + physics_loss + initial_condition_loss + boundary_condition_loss + reg_loss
...
def train_model(model, tensor_data, N, device, lr=1e-3, num_epochs=1000, early_stopping_patience=10):
    loss_history = []

    model_optimizer = optim.Adam(model.parameters(), lr=lr)
    early_stopping = EarlyStopping(patience=early_stopping_patience, verbose=True)
    model_scheduler = StepLR(model_optimizer, step_size=5000, gamma=0.99)
...
model = SIRDNet(inverse=True, init_beta=0.1, init_gamma=0.05, init_mu=0.01, retain_seed=42, num_layers=4, hidden_neurons=20).to(device)

# Train the model
N = data["population"].values[0]
trained_model, loss_history = train_model(model, tensor_data, N, device, lr=1e-3, num_epochs=10000, early_stopping_patience=50)
...
path_results = '../../reports/images/'

def plot_result_comparation(region, pred_data, data_type, real_data, train_size):
    if not os.path.exists(path_results):
        os.makedirs(path_results)

    plt.figure(figsize=(10, 5))
    t = np.linspace(0, len(pred_data), len(pred_data) + 1)[:-1]
    ...
    plt.plot(t, pred_data, color='red', label=f'{data_type}_PINNs')

    plt.xlabel('Days')
    plt.ylabel(f'{data_type}')
    plt.title(f'{region} {data_type} Prediction')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(path_results, f'{region}_{data_type}_{train_size}_result.pdf'), dpi=600)
    plt.show()

# Prepare the data for plotting pred and real data
