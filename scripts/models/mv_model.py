...
metrics['Time elapsed'] = It.elapsed
metric_record.append(metrics)
y_pred = format_y_pred(y_pred, 'Exponential Smoothing prediction')
pred_df = pred_df.join(y_pred)

fig = plot_forecast(pred_df, ['Exponential Smoothing prediction'], ['Exponential Smoothing prediction'])
fig = format_plot(fig, title=f"Exponential Smoothing: MAE={metrics['MAE']:.2f} | MSE={metrics['MSE']:.2f} | MASE={metrics['MASE']:.2f} | Forecast Bias={metrics['Forecast Bias']:.2f}")
fig.update_xaxes(type='date', rangeslider_visible=True)
fig.show()
